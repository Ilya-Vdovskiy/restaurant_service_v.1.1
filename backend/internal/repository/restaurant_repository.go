...
func (r *RestaurantRepository) Deactivate(ctx context.Context, id string) (models.Restaurant, error) {
	const query = `
		UPDATE restaurants
		SET is_active = false
		WHERE id = $1
	...
	if err != nil {
		...
		return models.Restaurant{}, fmt.Errorf("deactivate restaurant by id: %w", err)
	}
